...
int main() {
    ...
    getline(f, line);
    int count, start, end, stackSize; string _;
    while (true){
        f >> _ >> count >> _ >> start >> _ >> end;
        stackSize = stacks[start-1].size();
        for (int i=0; i<count; i++) {
            stacks[end-1].push_back(stacks[start-1][stackSize-count]);
            stacks[start-1].erase(stacks[start-1].begin()+(stackSize-count));
        }
        if (f.eof()) break;
    }
